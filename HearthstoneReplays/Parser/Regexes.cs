...
namespace HearthstoneReplays.Parser
{
  ...
  //          new Regex(@"id=(\d+) PlayerId=(\d+) ChoiceType=(\w+) CountMin=(\d+) CountMax=(\d+)$");
		...
		public static readonly Regex SubSpellEndRegex = new Regex(@"SUB_SPELL_END.*$");
		public static readonly Regex SubSpellSourceRegex = new Regex(string.Format(@"Source = {0}$", Entity));
		public static readonly Regex SubSpellTargetsRegex = new Regex(string.Format(@"Targets\[\d+\] = {0}$", Entity));
