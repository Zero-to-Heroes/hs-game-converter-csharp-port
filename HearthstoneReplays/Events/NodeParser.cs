    ...
    public class NodeParser
    {
        ...
        public void EnqueueGameEvent(List<GameEventProvider> providers)
        {
            ...
            lock (listLock)
            {
                ...
                var priorityProviders = providers.Where(p => p.EventName == "MATCH_METADATA").ToList();
                if (priorityProviders.Count > 0)
                {
                    ProcessGameEvent(priorityProviders[0]);
                    metadataEmit = true;
                    return;
                }
        ...
        private async void ProcessGameEventQueue(Object source, ElapsedEventArgs e)
        {
            ...
            while (IsEventToProcess())
            {
                ...
                try
                {
                    ...
                    //}
                    //if (!this.metadataEmit)
                    //{
                    //    Logger.Log("[csharp] waiting for metadata", provider.CreationLogLine);
                    ...
                    //Logger.Log("[csharp] metadata " + ParserState.CurrentGame.FormatType
                    //    + " // " + ParserState.CurrentGame.GameType
                    //    + " // " + ParserState.LocalPlayer?.Name, "");
                    //if (provider.debug)
                    //{
